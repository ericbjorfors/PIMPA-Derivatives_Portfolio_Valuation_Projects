...
#value is a list with the pricer object and a list with the trade_id that corresponds to it e.g. {'IRS':[IRS_pricing_object,[123,124]]}
        ...
        for date in self.simulation_dates:
            ...
            if date in self.b3_default_grid:
                ...
                for quantile in self.pe_quantiles:
                    self.uncollateralised_pe[quantile][0,count_grid_default] = np.quantile(np.maximum(self.scenarios_portfolio_values[:,count_array],0), q=quantile, axis=0)
    ...
    def run(self, today_date, global_parameters, pe_quantiles=[0.99], mpor_d=14):
        ...
        self.scenarios = self.generator.generate_scenarios(
    ...
    def get_exposures__(self):
        # Force everything to np.ndarray before indexing
        ee_array = np.asarray(self.uncollateralised_ee)
        result = pd.DataFrame({
            'default_times': self.b3_default_grid,
            'uncollateralized_ee': ee_array[0]
        })

        for quantile, values in self.uncollateralised_pe.items():
            pe_array = np.asarray(values)
            result[f'uncollateralized_pe_{quantile}'] = pe_array[0]

        if len(self.portfolio.vm_collateral_agreements) > 0:
            coll_ee_array = np.asarray(self.collateralised_ee)
            result['collateralized_ee'] = coll_ee_array[0]
            for quantile, values in self.collateralised_pe.items():
                coll_pe_array = np.asarray(values)
                result[f'collateralized_pe_{quantile}'] = coll_pe_array[0]
